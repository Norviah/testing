...
export type Data = SavedPermitDataStructure<'Easton', 'MA'>;
...
export async function main(): Promise<void> {
  ...
  await page.goto('https://permiteyes.us/easton/publicview.php', {
    timeout: 0,
  });
  ...
  writeFileSync(paths.EASTON_DATA, JSON.stringify(exported, null, 2));
}
...
async function readTable(
  ...
  while (rowCount < rows!.length) {
    ...
    const data: Data = { permitnumber: permitNumber!, city: 'Easton', state: 'MA' };
