...
export type Data = SavedPermitDataStructure<'Hingham', 'MA'>;
...
export async function main(): Promise<void> {
  ...
  await page.goto('https://permiteyes.us/hingham/publicview.php', {
    timeout: 0,
  });
  ...
  if (!existsSync(paths.HINGHAM)) {
    mkdirSync(paths.HINGHAM, { recursive: true });
  }

  writeFileSync(paths.HINGHAM_DATA, JSON.stringify(exported, null, 2));
}

async function readTable(
  page: puppeteer.Page,
  tableBodyy: ElementHandle<Element> | null,
): Promise<boolean> {
  //   let rows = await tableBody?.$$('tr');
  //   let rowCount = 0;
  const table = await page.$('.dataTables_scrollBody');
  const tableBody = await table?.$('tbody');
  let rows = await tableBody?.$$('tr');
  let rowCount = 0;
  ...
  while (rowCount < rows!.length) {
    ...
    const data: Data = { permitnumber: permitNumber!, city: 'Hingham', state: 'MA' };
