...
export type Data = SavedPermitDataStructure<'Foxborough', 'MA'>;
...
export async function main(): Promise<void> {
  ...
  await page.goto('https://permiteyes.us/foxborough/publicview.php', {
    timeout: 0,
  });
  ...
  const range = await page.$('.daterangepicker');
  const ranges = await range?.$('ul');
  const li = await ranges?.$$('li');

  await li![3].click();

  await new Promise((resolve) => setTimeout(resolve, smallDelay));

  const table = await page.$('#PublicView');
  ...
  writeFileSync(paths.FOXBOROUGH_DATA, JSON.stringify(exported, null, 2));
}
...
async function readTable(
  ...
  while (rowCount < rows!.length) {
    ...
    const data: Data = { permitnumber: permitNumber!, city: 'Foxborough', state: 'MA' };
  ...
  const nextButton = await page.$('#PublicView_next');
