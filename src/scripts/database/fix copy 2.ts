...
import { type Agent, type City, Prisma, PrismaClient } from '@prisma/client';
...
async function main(): Promise<void> {
  ...
  const agentsToCreate: Prisma.AgentCreateManyInput[] = [];
  ...
  for (const entries of data) {
    ...
    agentsToCreate.push(createData);
  ...
  writeFileSync(join(__filename, '..', 'prisma.json'), JSON.stringify(agentsToCreate, null, 2));
  // writeFileSync(
